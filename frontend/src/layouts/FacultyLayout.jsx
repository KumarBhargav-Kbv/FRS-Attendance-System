...
import {
  LayoutDashboard, BookOpen, Play, Radio, History, FileBarChart,
  User, LogOut, Menu, ChevronLeft, ScanFace
} from 'lucide-react';

const sidebarLinks = [
  { path: '/faculty/dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { path: '/faculty/my-classes', label: 'My Classes', icon: BookOpen },
  { path: '/faculty/start-attendance', label: 'Start Attendance', icon: Play },
  { path: '/faculty/live-attendance', label: 'Live Attendance', icon: Radio },
  { path: '/faculty/attendance-history', label: 'Attendance History', icon: History },
    ...
    <div className="flex flex-col h-full">
      <div className="px-6 py-5 border-b border-surface-700/50">
        <div className="flex items-center gap-3">
          ...
          {sidebarOpen && (
            <div><h2 className="text-white font-bold text-sm">FRS Attendance</h2><p className="text-surface-400 text-xs">Faculty Panel</p></div>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="bg-white border-b border-surface-100 px-4 lg:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-medium text-surface-500">Welcome, <span className="text-surface-900">{user?.name}</span></span>
          </div>
