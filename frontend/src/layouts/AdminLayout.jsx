...
import {
  LayoutDashboard, Users, GraduationCap, Building2, BookOpen, DoorOpen,
  ScanFace, ClipboardCheck, FileBarChart, BarChart3, ScrollText, Settings,
...
const sidebarLinks = [
  ...
  { path: '/admin/classes', label: 'Classes', icon: DoorOpen },
  { path: '/admin/face-registration', label: 'Face Registration', icon: ScanFace },
...
export default function AdminLayout() {
        ...
        <div className="flex items-center gap-3">
          ...
          {sidebarOpen && (
            <div className="animate-fade-in">
              <h2 className="text-white font-bold text-sm">FRS Attendance</h2>
  ...
  return (
    <div className="flex h-screen bg-surface-50">
      {/* Desktop Sidebar */}
      <aside className={`hidden lg:flex flex-col bg-surface-900 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'}`}>
        <SidebarContent />
      </aside>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="absolute left-0 top-0 h-full w-64 bg-surface-900 shadow-2xl">
            <SidebarContent />
          </aside>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b border-surface-100 px-4 lg:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="hidden lg:flex p-2 hover:bg-surface-100 rounded-lg">
              <ChevronLeft className={`w-5 h-5 text-surface-600 transition-transform ${!sidebarOpen ? 'rotate-180' : ''}`} />
            </button>
            <span className="text-sm font-medium text-surface-500">Welcome back, <span className="text-surface-900">{user?.name}</span></span>
          </div>
